...
% load T20_finalData.mat;
% X = [];
% len = size(cell_T20_point,2);
% for i = 1:len
%     X = [X;cell_T20_point{2,i}];
% end

% load T15_finalData.mat;
% X = [];
% len = size(cell_T15_point,2);
% for i = 1:len
%     X = [X;cell_T15_point{2,i}];
% end
...
options = statset('Display','final','MaxIter',100,'TolFun',1e-4);
for k = 1:numComlim
