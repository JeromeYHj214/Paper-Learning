...
for i = 1:len
    ...
    if (X_test{5,i} < X_test{7,i} && X_test{6,i} < X_test{7,i} && X_test{8,i} < X_test{7,i})
        count5 = count5 + 1;
    elseif(X_test{6,i} < X_test{5,i} && X_test{7,i} < X_test{5,i} && X_test{8,i} < X_test{5,i})
        count3 = count3 + 1;
    elseif(X_test{5,i} < X_test{6,i} && X_test{7,i} < X_test{6,i} && X_test{8,i} < X_test{6,i})
        count4 = count4 + 1;
    else
        count6 = count6 + 1;
    end
...
figure;
set(gcf,'color','white');
data = [count3 count4 count5 count6];
b=bar(data);
set(gca,'XTickLabel',{'三分布元','四分布元','五分布元','六分布元'})
set(gca,'YLim',[0 40])
ylabel('样本个数（个）');
